...
const PremiumBadge = () => (
  <span className="ml-1 inline-flex h-4 w-4 items-center justify-center rounded-full bg-red-500">
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <header className="flex items-center justify-between px-6 py-4">
        <img src={logo} alt="ClothesFriends" className="h-10 w-auto object-contain" style={{ mixBlendMode: "multiply" }} />
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-6">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-16 text-center">
          <p className="text-sm text-muted-foreground">Benvenuto</p>
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-foreground">Ciao, {username}</h1>
        </motion.div>

        {/* ElevenLabs Agent - Closie */}
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.2, ease: "easeOut" }} className="relative">
          {conversation.status === "disconnected" ? (
            <button onClick={startConversation} className="animate-orb-pulse group relative flex h-52 w-52 items-center justify-center rounded-full border-2 border-primary/30 bg-background transition-all duration-300 hover:border-primary hover:shadow-lg active:scale-95">
              <div className="animate-orb-breathe flex flex-col items-center gap-2">
                ...
                <span className="text-xs font-medium text-muted-foreground group-hover:text-foreground transition-colors">Clicca qui per parlare</span>
                <span className="text-xs text-muted-foreground/60">con Closie</span>
              </div>
            </button>
          ) : (
            <button onClick={stopConversation} className="group relative flex h-52 w-52 items-center justify-center rounded-full border-2 border-primary bg-primary/5 transition-all duration-300 hover:bg-primary/10 active:scale-95">
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-xs font-medium text-primary">{conversation.isSpeaking ? "Closie sta parlando..." : "In ascolto..."}</span>
      ...
      <AnimatePresence>
        {selectedImages.length > 0 && !galleryOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="border-t border-border bg-secondary/30 px-4 py-2">
              ...
              {selectedImages.map((id) => {
                ...
                return (
                  <div key={id} className="relative shrink-0">
                    ...
                    <button onClick={() => setSelectedImages((p) => p.filter((i) => i !== id))} className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-foreground text-background">
                      <X size={12} />
                    </button>
                  </div>
                );
      ...
      {/* Bottom Input Bar */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }} className="border-t border-border bg-background px-4 py-3">
      ...
      <AnimatePresence>
        {creditCardOpen && (
          <>
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 28, stiffness: 300 }} className="fixed bottom-0 left-0 right-0 z-50 rounded-t-2xl bg-background px-6 pb-8 pt-4">
              <div className="mb-4 flex justify-center"><div className="h-1 w-10 rounded-full bg-border" /></div>
              ...
              <div className="space-y-4">
                ...
                <button onClick={() => setCreditCardOpen(false)} className="w-full rounded-xl bg-foreground py-3.5 text-sm font-semibold text-background transition-opacity hover:opacity-90">Salva</button>
              </div>
            </motion.div>
          </>
        )}
      ...
      <AnimatePresence>
        {profileOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-foreground/40" onClick={() => setProfileOpen(false)} />
            <motion.div
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
              transition={{ type: "spring", damping: 28, stiffness: 300 }}
              className="fixed right-0 top-0 bottom-0 z-50 w-72 bg-background shadow-xl flex flex-col"
            >
              <div className="flex items-center justify-between border-b border-border px-5 py-4">
                <h3 className="text-base font-bold text-foreground">Profilo</h3>
                <button onClick={() => setProfileOpen(false)} className="text-muted-foreground hover:text-foreground"><X size={20} /></button>
              </div>
              <div className="flex-1 overflow-y-auto py-2">
                {/* Wardrobe */}
                <button onClick={() => { setProfileOpen(false); setWardrobeOpen(true); }} className="flex w-full items-center gap-4 px-5 py-3.5 text-sm font-medium text-foreground hover:bg-secondary transition-colors">
                  <span className="flex h-9 w-9 items-center justify-center rounded-full bg-foreground text-background">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="2" width="18" height="20" rx="1" /><line x1="12" y1="2" x2="12" y2="22" /><line x1="9" y1="10" x2="9" y2="14" /><line x1="15" y1="10" x2="15" y2="14" /></svg>
                  </span>
                  Armadio virtuale
                </button>
                {/* Credit Card */}
                <button onClick={() => { setProfileOpen(false); setCreditCardOpen(true); }} className="flex w-full items-center gap-4 px-5 py-3.5 text-sm font-medium text-foreground hover:bg-secondary transition-colors">
                  <span className="flex h-9 w-9 items-center justify-center rounded-full bg-foreground text-background"><CreditCard size={16} /></span>
                  Carta di credito
                </button>
                {/* Calendar (premium) */}
                <button onClick={() => navigate("/premium")} className="flex w-full items-center gap-4 px-5 py-3.5 text-sm font-medium text-foreground hover:bg-secondary transition-colors">
                  <span className="flex h-9 w-9 items-center justify-center rounded-full bg-foreground text-background"><Calendar size={16} /></span>
                  Calendario<PremiumBadge />
                </button>
                {/* Settings */}
                <button className="flex w-full items-center gap-4 px-5 py-3.5 text-sm font-medium text-foreground hover:bg-secondary transition-colors">
                  <span className="flex h-9 w-9 items-center justify-center rounded-full bg-foreground text-background"><Settings size={16} /></span>
                  Impostazioni
                </button>
              </div>
            </motion.div>
